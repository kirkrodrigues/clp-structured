...
namespace clp_structured { namespace search {
    void Output::filter() {
        ...
        for (auto const& archive : ReaderUtils::get_archives(m_archive_dir)) {
            ...
            // range index
            EvaluateTimestampIndex timestamp_index(
                    ReaderUtils::read_local_timestamp_dictionary(archive)
            );
    ...
    Output::constant_propagate(std::shared_ptr<Expression> const& expr, int32_t schema_id) {
        if (std::dynamic_pointer_cast<OrExpr>(expr)) {
            ...
            } else if (filter->get_column()->is_pure_wildcard() && filter->get_column()->matches_any(LiteralType::ClpStringT | LiteralType::VarStringT))
            {
                ...
                bool valid = filter->get_operand()->as_var_string(
                                     filter_string,
